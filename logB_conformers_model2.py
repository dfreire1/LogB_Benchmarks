...
def calculate_delta_g(gibbs_values, g1, g1s, g2, g2s, g3, g3s, g4, g4s):
    ...
    G4 = float(gibbs_values.get(g4, 0))
    G4S = float(gibbs_values.get(g4s, 0))

    delta_g_au = ((G3 + 5*G4 - G1 - G2) + (G3S - G3) + 5*(G4S - G4)) - (G1S - G1) - (G2S - G2)
    return delta_g_au

def calculate_delta_h(enthalpy_values, h1, h1s, h2, h2s, h3, h3s, h4, h4s):
    ...
    H4 = float(enthalpy_values.get(h4, 0))
    H4S = float(enthalpy_values.get(h4s, 0))

    delta_h_au = ((H3 + 5*H4 - H1 - H2) + (H3S - H3) + 5*(H4S - H4)) - (H1S - H1) - (H2S - H2)
    return delta_h_au
...
data_directory = '/Users/dfreire/Documents/TCU/Green_Group/Projects/8_LogB_Benchmarks/Calculations/03_Conformers/Zn/ZnL08a_Crest/F05'
...
for codename in codenames:
    for x in range(1, 39):  # Modify range if needed
        ...
        for y in range(1, 5):  # Modify range if needed
            formatted_y = format_file_number(y)
            combination = f"L{formatted_x}_ML{formatted_y}"

            # Construct file names with codename
            files = [
                f"Zn_{codename}_of", f"Zn_{codename}_smd_of",
                f"L08C_{codename}_conf{formatted_x}", f"L08C_{codename}_smd_conf{formatted_x}",
                f"ZnL08Ca_{codename}_conf{formatted_y}", f"ZnL08Ca_{codename}_smd_conf{formatted_y}",
                f"W_{codename}_of", f"W_{codename}_smd_of"
            ]

            # Debugging for A02 L08_ML08
            #if codename == any and combination == "L01_ML01":
            if combination == "L01_ML01":
                example_values = {file: gibbs_values.get(file, None) for file in files}
...
# Save to Excel file
with pd.ExcelWriter(os.path.join(data_directory or os.getcwd(), "ZnL08_F05_Model2.xlsx"), engine='xlsxwriter') as writer:
